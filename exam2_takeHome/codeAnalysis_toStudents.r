...
#    It is better practice to only rescale numeric features...
...
(S_lambdamin = predict(out.glmnet,s='lambda.min',type='nonzero')[,1])
...
#############
#look at selected important vars vs actual
#############
load('importantFeatures.Rdata')
(S_lambdamin = predict(out.glmnet,s='lambda.min',type='nonzero')[,1])
sort(importantFeatures)+1#the + 1 is due to the intercept
